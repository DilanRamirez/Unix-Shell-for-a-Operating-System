...
int tokenizeLine(char* line, int length, char** args){
    int i = 0;
...
void executeCommands(char** args){
    ...
    if(strcmp(args[0], "exit") == 0) { //takes the args[0] which is the token "exit" to finish the program
        ...
        if(chdir(args[1]) != 0) { //chdir() is in charge of accessing to the directory assigned by args[1]
            write(1, "Invalid Directory", strlen("Invalid Directory")); //a success value is 0
        }
        return;
    }
...
void loop(){
    ...
    while(true){
        ...
        tokenizeLine(input_line, len, args);
